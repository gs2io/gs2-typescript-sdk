...
export default class DescribeNamespacesResult implements IResult {
    ...
    public getItems(): Gs2Log.Namespace[]|null {
        return this.items;
    }

    public setItems(items: Gs2Log.Namespace[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Log.Namespace[]|null): this {
        this.items = items;
        return this;
    }
        ...
        return new DescribeNamespacesResult()
            .withItems(data.items ?
                data.items.map((item: {[key: string]: any}) => {
                    return Gs2Log.Namespace.fromDict(item);
                }
            ) : null)
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Log.Namespace) => {
                    return item.toDict();
                }
            ) : null,
