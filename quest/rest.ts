...
export default class Gs2QuestRestClient extends AbstractGs2RestClient {
    ...
    public createQuestModelMaster(request: Request.CreateQuestModelMasterRequest): Promise<Result.CreateQuestModelMasterResult> {
        ...
        const body: {[key: string]: any} = {
            ...
            'description': request.getDescription() ?? null,
            'metadata': request.getMetadata() ?? null,
            'contents': request.getContents()?.map((item) => item.toDict()) ?? null,
            'challengePeriodEventId': request.getChallengePeriodEventId() ?? null,
            'firstCompleteAcquireActions': request.getFirstCompleteAcquireActions()?.map((item) => item.toDict()) ?? null,
            ...
            'failedAcquireActions': request.getFailedAcquireActions()?.map((item) => item.toDict()) ?? null,
            'premiseQuestNames': request.getPremiseQuestNames() ?? null,
        };
        return axios.post(
            url,
            body,
    ...
    public createProgressByUserId(request: Request.CreateProgressByUserIdRequest): Promise<Result.CreateProgressByUserIdResult> {
        ...
        const body: {[key: string]: any} = {
            'contextStack': request.getContextStack() ?? null,
            'questModelId': request.getQuestModelId() ?? null,
