...
export default class DescribeWalletsByUserIdResult implements IResult {
    ...
    public getItems(): Gs2Money.Wallet[]|null {
        return this.items;
    }

    public setItems(items: Gs2Money.Wallet[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Money.Wallet[]|null): this {
        this.items = items;
        return this;
    }
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Money.Wallet) => {
                    return item.toDict();
                }
            ) : [],
