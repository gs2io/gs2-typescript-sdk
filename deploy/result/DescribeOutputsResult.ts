...
export default class DescribeOutputsResult implements IResult {
    ...
    public getItems(): Gs2Deploy.Output[]|null {
        return this.items;
    }

    public setItems(items: Gs2Deploy.Output[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Deploy.Output[]|null): this {
        this.items = items;
        return this;
    }
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Deploy.Output) => {
                    return item.toDict();
                }
            ) : [],
