...
export default class DescribeRoomsResult implements IResult {
    ...
    public getItems(): Gs2Chat.Room[]|null {
        return this.items;
    }

    public setItems(items: Gs2Chat.Room[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Chat.Room[]|null): this {
        this.items = items;
        return this;
    }
        ...
        return new DescribeRoomsResult()
            .withItems(data.items ?
                data.items.map((item: {[key: string]: any}) => {
                    return Gs2Chat.Room.fromDict(item);
                }
            ) : [])
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Chat.Room) => {
                    return item.toDict();
                }
            ) : [],
