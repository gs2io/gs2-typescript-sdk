...
export default class DescribeStatusesByUserIdResult implements IResult {
    private items: Gs2Idle.Status[]|null = null;
    private nextPageToken: string|null = null;

    public getItems(): Gs2Idle.Status[]|null {
        return this.items;
    }

    public setItems(items: Gs2Idle.Status[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Idle.Status[]|null): this {
        this.items = items;
        return this;
    }
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Idle.Status) => {
                    return item.toDict();
                }
            ) : null,
