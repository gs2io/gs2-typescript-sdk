...
export class DescribeKeysResult implements IResult {
    ...
    public getItems(): Gs2Key.Key[]|null {
        return this.items;
    }

    public setItems(items: Gs2Key.Key[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Key.Key[]|null): this {
        this.items = items;
        return this;
    }
    ...
    public static fromDict(data: {[key: string]: any}): DescribeKeysResult {
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Key.Key) => {
                    return item.toDict();
                }
            ) : [],
