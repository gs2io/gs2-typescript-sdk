...
export class DescribeEntryModelsResult implements IResult {
    ...
    public setItems(items: Gs2Dictionary.EntryModel[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Dictionary.EntryModel[]|null): this {
        this.items = items;
        return this;
    }

    public static fromDict(data: {[key: string]: any}): DescribeEntryModelsResult {
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Dictionary.EntryModel) => {
                    return item.toDict();
                }
            ) : [],
        };
