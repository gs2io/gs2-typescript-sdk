...
export class EntryAccessTokenDomain {
    ...
    public async load(
        ...
        let r: GetEntryResult = await this.client.getEntry(
            request
        );
    ...
    ): Promise<GetEntryWithSignatureResult> {
        request.setNamespaceName(this.namespaceName);
        request.setAccessToken(this.accessToken != null ? this.accessToken.getToken() : null);
        request.setEntryModelName(this.entryModelName);
        let r: GetEntryWithSignatureResult = await this.client.getEntryWithSignature(
            request
        );
        this.entryCache.update(r.getItem()!);
        return r;
