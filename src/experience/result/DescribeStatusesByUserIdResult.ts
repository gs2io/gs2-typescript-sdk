...
export class DescribeStatusesByUserIdResult implements IResult {
    ...
    public getItems(): Gs2Experience.Status[]|null {
        return this.items;
    }

    public setItems(items: Gs2Experience.Status[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Experience.Status[]|null): this {
        this.items = items;
        return this;
    }
        ...
        return new DescribeStatusesByUserIdResult()
            .withItems(data.items ?
                data.items.map((item: {[key: string]: any}) => {
                    return Gs2Experience.Status.fromDict(item);
                }
            ) : [])
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Experience.Status) => {
                    return item.toDict();
                }
            ) : [],
