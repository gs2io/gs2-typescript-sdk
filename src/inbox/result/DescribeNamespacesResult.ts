...
export default class DescribeNamespacesResult implements IResult {
    ...
    public getItems(): Gs2Inbox.Namespace[]|null {
        return this.items;
    }

    public setItems(items: Gs2Inbox.Namespace[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Inbox.Namespace[]|null): this {
        this.items = items;
        return this;
    }
        ...
        return new DescribeNamespacesResult()
            .withItems(data.items ?
                data.items.map((item: {[key: string]: any}) => {
                    return Gs2Inbox.Namespace.fromDict(item);
                }
            ) : [])
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Inbox.Namespace) => {
                    return item.toDict();
                }
            ) : [],
