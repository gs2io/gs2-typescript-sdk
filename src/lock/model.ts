...
export class Namespace implements IModel {
  ...
  public static createGrn(region: string, ownerId: string, namespaceName: string): string {
    return 'grn:gs2:{region}:{ownerId}:lock:{namespaceName}'
  ...
  public static getRegionFromGrn(grn: string) {
    let grnFormat = 'grn:gs2:{region}:{ownerId}:lock:{namespaceName}';
...
export class Mutex implements IModel {
  ...
  public static createGrn(region: string, ownerId: string, namespaceName: string, userId: string, propertyId: string): string {
    return 'grn:gs2:{region}:{ownerId}:lock:{namespaceName}:user:{userId}:property:{propertyId}'
      .replace('{region}', region)
      .replace('{ownerId}', ownerId)
      .replace('{namespaceName}', namespaceName)
  ...
  public static getRegionFromGrn(grn: string) {
    let grnFormat = 'grn:gs2:{region}:{ownerId}:lock:{namespaceName}:user:{userId}:property:{propertyId}';
    grnFormat = grnFormat.replace('{region}', '(.*)');
    grnFormat = grnFormat.replace('{ownerId}', '(.*)');
    grnFormat = grnFormat.replace('{namespaceName}', '(.*)');
  ...
  constructor(
    ...
    if (data && data.mutexId !== undefined) {
      this.mutexId = data.mutexId;
    } else {
      this.mutexId = undefined;
    }
    ...
    if (data && data.referenceCount !== undefined) {
    ...
    if (data && data.ttlAt !== undefined) {
      this.ttlAt = data.ttlAt;
    } else {
      this.ttlAt = 0;
    }
  }

  public withMutexId(mutexId?: string): this {
  ...
  public withReferenceCount(referenceCount?: number): this {
  ...
  public withTtlAt(ttlAt?: number): this {
    this.ttlAt = ttlAt;
    return this;
  }
