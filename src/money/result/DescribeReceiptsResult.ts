...
export class DescribeReceiptsResult implements IResult {
    ...
    public getItems(): Gs2Money.Receipt[]|null {
        return this.items;
    }

    public setItems(items: Gs2Money.Receipt[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2Money.Receipt[]|null): this {
        this.items = items;
        return this;
    }
        ...
        return new DescribeReceiptsResult()
            .withItems(data.items ?
                data.items.map((item: {[key: string]: any}) => {
                    return Gs2Money.Receipt.fromDict(item);
                }
            ) : [])
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2Money.Receipt) => {
                    return item.toDict();
                }
            ) : [],
