...
var grnFormat = "grn:gs2:{region}:{ownerId}:news:{namespaceName}:progress:{uploadToken}:output:{outputName}";
var Output = /** @class */ (function () {
    ...
    Output.getNamespaceName = function (grn) {
        var match = grn.match(grnFormat
            .replace('{region}', '.*')
            .replace('{ownerId}', '.*')
            .replace('{namespaceName}', '(.*)')
    ...
    Output.getUploadToken = function (grn) {
        var match = grn.match(grnFormat
            .replace('{region}', '.*')
            .replace('{ownerId}', '.*')
            .replace('{namespaceName}', '.*')
    ...
    Output.createGrn = function (region, ownerId, namespaceName, uploadToken, outputName) {
        return grnFormat
