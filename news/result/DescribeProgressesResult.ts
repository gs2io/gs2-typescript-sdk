...
export default class DescribeProgressesResult implements IResult {
    private items: Gs2News.Progress[]|null = null;
    private nextPageToken: string|null = null;

    public getItems(): Gs2News.Progress[]|null {
        return this.items;
    }

    public setItems(items: Gs2News.Progress[]|null) {
        this.items = items;
        return this;
    }

    public withItems(items: Gs2News.Progress[]|null): this {
        this.items = items;
        return this;
    }
    ...
    public static fromDict(data: {[key: string]: any}): DescribeProgressesResult {
    ...
    public toDict(): {[key: string]: any} {
        return {
            "items": this.getItems() ?
                this.getItems()!.map((item: Gs2News.Progress) => {
                    return item.toDict();
                }
            ) : null,
